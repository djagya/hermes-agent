...
) -> tuple[Optional[tuple], Optional[str], object]:
    """Monitor gate (hash-suppressed change detection). Must run BEFORE any agent machinery so an
    unchanged tick costs no LLM/delivery. Returns ``(early_result | None, extra_prompt, pending)``;
    when early_result is None, extra_prompt may carry the injected monitor context and pending
    is the changed outcome to commit after later gates.
    """
...
def _prepare_job_prompt(
...
) -> tuple[Optional[_RunResult], Optional[str], object]:
    """Run every pre-agent gate and build the prompt. Returns ``(early_result, prompt, pending_monitor)``."""
    ...
    _early, extra_prompt, pending_monitor = _apply_monitor_gate(job, job_id, job_name, extra_prompt)
...
def run_job(
    ...
    early, prompt, pending_monitor = _prepare_job_prompt(job, job_id, job_name, extra_prompt, cancel_event)
