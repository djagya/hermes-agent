...
_TELEGRAM_ALBUM_IMAGE_EXTS = {".jpg", ".jpeg", ".png", ".webp"}
...
async def _send_telegram_photo_album(bot, chat_id, image_paths, media_kwargs):
    ...
    if None in group_ids or len(group_ids) != 1:
        raise RuntimeError("Telegram album receipts did not share one media_group_id")
    return receipts[-1]
    ...
    try:
        ...
        _album_paths = [
            ...
            if (
                ...
                and os.path.splitext(media_path)[1].lower() in _TELEGRAM_ALBUM_IMAGE_EXTS
            )
        ]
        _send_as_albums = (
            not force_document
        ...
        if _send_as_albums:
            try:
                ...
                warning = _sanitize_error_text(f"Failed to send Telegram photo album: {e}")
