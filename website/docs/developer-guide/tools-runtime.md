...
3. **Approval prompt** — if a match is found:
   ...
   - **Smart approval** — optionally, an auxiliary LLM can auto-approve low-risk commands that match patterns (e.g., `rm -rf node_modules/` is safe but matches "recursive delete"). The guardian's non-decision outcomes (blank, truncated, or unrecognized response, provider error) escalate to the human gate with a structured outcome category logged at WARNING (`empty_response`, `unrecognized_response`, `length_limited_response`, `uncertain`, `provider_exception`). `execute_code` cells additionally carry bounded, statically parsed provenance for helper names defined in earlier cells of the session kernel (never evaluated, redacted, size-capped), and every result of the human gate records `decision_source` (`smart`/`human`/`unattended`) plus `gate_id` so logs answer who decided; hardline, allowlist, and floor resolutions that end before the gate do not carry the tag.
