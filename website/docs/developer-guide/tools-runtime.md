...
2. **Target** — EVERY resolvable target the run could exercise: the runner's explicit path arguments (anchored at the cwd each command runs in), the `cd`-derived directories, the `workdir` parameter, and the session cwd. Recorded `VAR=value` assignments are substituted into every parsed word (`R=<repo>; make -C $R test` resolves exactly like `cd "$R" && pytest`), and the operand of a Hermes-specific runner script (`bash <checkout>/scripts/run_tests.sh`) anchors identity on the script's own location. Direct execution of a test file (`python tests/test_x.py`, `./path/to/test_x.py`, `python -c "import pytest; pytest.main([...]"`) is classified the same way as `pytest <path>`. The run blocks when ANY target resolves into a Hermes checkout: the running source root, its `.worktrees/*`, or any directory whose `pyproject.toml` declares `name = "hermes-agent"`.
