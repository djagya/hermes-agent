...
2. **Target** — EVERY resolvable target the run could exercise: the runner's explicit path arguments (anchored at the cwd each command runs in), the `cd`-derived directories, the `workdir` parameter, and the session cwd. The run blocks when ANY of them resolves into a Hermes checkout: the running source root, its `.worktrees/*`, or any directory whose `pyproject.toml` declares `name = "hermes-agent"`.
