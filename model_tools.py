...
_VAULT_NO_PASSWORD_NOTE = (" Vault note: on a login/checkout form call browser_vault_list first, then browser_vault_fill, or "
                           ...
                           "For a one-time / 2FA code call browser_vault_enter_code — pass a code you already hold (e.g. read "
                           "from the user's mail through Hermes' mail tools) together with expected_origin, or call it with "
                           "the login handle for a saved authenticator key. Never type a password, card number, CVC or "
