...
class TestTurnRunner:
    ...
    def test_preserves_interim_media_directives_in_turn_result(self, tmp_path):
        image_path = tmp_path / "interim.png"
        image_path.write_bytes(b"image")

        class _InterimMediaAgent:
            def __init__(self, **kwargs):
                self.model = kwargs["model"]
                self.session_id = kwargs["session_id"]
                self.tools = []
