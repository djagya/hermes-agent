...
KANBAN_GUIDANCE = (
    ...
    "user will unblock with context and the dispatcher will respawn you. Supply the actual `kind` and, when "
    "the installed tool schema supports it, a stable non-secret `blocker_key` (1–128 characters) identifying "
    "the obstacle, not its wording or broad category. Reuse it for the same cause; never rotate keys to evade "
    "escalation. Different causes within capability are not the same blocker.\n"
    ...
    "- **Large payloads cross boundaries by artifact, never inline.** If a body/prompt exceeds a short paragraph, "
    "write it to an immutable artifact and make the card body a digest contract: exact artifact path, SHA-256, byte "
    "length, and the instruction that the artifact bytes are the only authoritative contract (helper: "
    "`tools/durable_source.py::contract_text`). After `kanban_create`, read the artifact back and verify the digest; "
    "a preview or quote is never the contract, and replayed compacted tool-call arguments are never source authority.\n"
